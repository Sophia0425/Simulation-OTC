...
double simulate_menopause_age_population(mt19937 &rng, std::ofstream *traj_file) {
    ...
    int removed_amount_N2 = 0;
  
    const double p_target = 1e-3;
    const double dt_max   = 0.25;
    ...
    while (N1 + N2 > menopause_threshold) {
        ...
        if (dt <= EPS) {
            ...
            if (!removed_done && std::abs(age - removal_age) < 1e-9) {
                // REMOVE ONLY FROM N2
                removed_amount_N2 = static_cast<int>(std::round(removal_fraction * static_cast<double>(N1+N2)));
                if (removed_amount_N2 > N2) removed_amount_N2 = N2;
                if (removed_amount_N2 < 0) removed_amount_N2 = 0;

                N2 -= removed_amount_N2;
                removed_done = true;

            } else if (removed_done && !returned_done && std::abs(age - return_age) < 1e-9) {
                // RETURN ONLY INTO N2 
                int return_amount = static_cast<int>(std::round(return_fraction_of_removed * removed_amount_N2));
